...
const forageGetItem = async <T>(key: string, callback?: (err: unknown) => void) => {
  try {
...
const forageSetItem = async <T>(key: string, data: T,  callback?: (err: unknown) => void) => {
  try {
...
export default boot(async ({ app, redirect, router }) => {
  ...
  const authUserData = forageGetItem(StorageNamesEnum.AUTH_USER_DATA);
  if (!authUserData) {
    ...
    if (!['/auth/signin', '/'].includes(router.currentRoute.value.path)) {
      const authStore = forageGetItem<AuthUserData>(StorageNamesEnum.AUTH_USER_DATA) as AuthUserData;
